...
public class EmpSystem {
	...
	private int selectMenu() {
		...
		try {
			...
			//이 메세지를 보고나면 제대로 할 거라는 가정하에 아래 코딩
			//menu = Integer.parseInt(sc.nextLine());
		}
		return menu;
	}
	...
	private void selectEmployee() {
		...
		int employeeId = inputEmployeeId();
		//DB에 검색
		Employees emp = dao.selectOne(employeeId);
		//결과 출력
		if(emp != null) {
	...
	private Employees inputAll() {
		...
		emp.setHire_Date(Date.valueOf(sc.nextLine()));
		//데이터타입에 맞게 정확한 변환 필요
		System.out.print("직급>>");
		...
		System.out.print("상여>>");
		emp.setCommisionPct(Double.parseDouble(sc.nextLine()));
		System.out.print("상사>>");
	...
	private Employees inputUpdateInfo() {
		Employees emp = new Employees();
		System.out.print("사번>>");
		emp.setEmployeeId(Integer.parseInt(sc.nextLine()));
		System.out.print("연봉>>");
